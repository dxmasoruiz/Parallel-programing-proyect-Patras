...
int main(int argc, char *argv[]) {
    ...
    for (int i = 0; i < MAXVARS; i++) best_pt[i] = 0.0;
    ...
    #pragma omp parallel
    {
        double thread_best_fx = 1e10;
        double thread_best_pt[MAXVARS];
        int thread_best_trial = -1;
        int thread_best_jj = -1;

        for (int i = 0; i < MAXVARS; i++) thread_best_pt[i] = 0.0;

        #pragma omp for
        for (int trial = 0; trial < ntrials; trial++) {
            double trial_startpt[MAXVARS], trial_endpt[MAXVARS];
            double fx;
            int jj;

            // Inicializar puntos de inicio aleatorios
            for (int i = 0; i < nvars; i++) {
				unsigned int seed = omp_get_thread_num();  
        		double random_value = -4 + (rand_r(&seed) / (double)RAND_MAX) * 8;
            }

            jj = hooke(nvars, trial_startpt, trial_endpt, rho, epsilon, itermax);
            fx = f(trial_endpt, nvars);

            if (fx < thread_best_fx) {
                thread_best_fx = fx;
                thread_best_trial = trial;
                thread_best_jj = jj;
                for (int i = 0; i < nvars; i++) {
                    thread_best_pt[i] = trial_endpt[i];
                }
            }
        }

        // Reducir los resultados de los hilos al mejor global
        #pragma omp critical
        {
            if (thread_best_fx < best_fx) {
                best_fx = thread_best_fx;
                best_trial = thread_best_trial;
                best_jj = thread_best_jj;
                for (int i = 0; i < nvars; i++) {
                    best_pt[i] = thread_best_pt[i];
                }
            }
        }
    }
