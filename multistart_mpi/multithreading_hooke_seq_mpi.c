...
int main(int argc, char *argv[]) {
    int rank, size;
    double global_minimum = 1e10;
    double local_minimum = 1e10;
    double startpt[MAXVARS], endpt[MAXVARS];
    double best_pt[MAXVARS];
    unsigned long local_funevals = 0, global_funevals = 0;
    ...
    int local_points = NPOINTS / size;
    int extra_points = NPOINTS % size;
    if (rank < extra_points) local_points++;

    t_start = MPI_Wtime();

    for (int trial = 0; trial < local_points; trial++) {
        for (int i = 0; i < MAXVARS; i++) {
            startpt[i] = 8.0 * drand48() - 4.0;
        }

        hooke(MAXVARS, startpt, endpt, RHO_BEGIN, EPSMIN, IMAX);

        double fx = f(endpt, MAXVARS);
        if (fx < local_minimum) {
            local_minimum = fx;
            for (int i = 0; i < MAXVARS; i++) {
                best_pt[i] = endpt[i];
    ...
    MPI_Reduce(&funevals, &global_funevals, 1, MPI_UNSIGNED_LONG, MPI_SUM, 0, MPI_COMM_WORLD);
    ...
    if (rank == 0) {
        ...
        printf("Global minimum: f(x) = %15.7le\n", global_minimum);
    }
